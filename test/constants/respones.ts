export const RESPONSE = {
  ...
  FILE_SIZE_TOO_BIG: { code: '1006', message: 'File size is too big.' },
  UPLOAD_FILE_FAILED: { code: '1007', message: 'Upload File Failed!.' },
  ...
  CHANGE_USERNAME_30_DAYS: {
    code: '1017',
    message: 'Change Username: requires minimum 30 days.',
  },
  CHANGE_USERNAME_SAME_NAME: {
    code: '1018',
    message: 'Change Username: same other name.',
  },

  // Auth error
  SPAM: { code: '9991', message: 'Spam.' },
  PRODUCT_NOT_EXISTED: { code: '9992', message: 'Product is not existed.' },
  CODE_VERIFY_INCORRECT: { code: '9993', message: 'Code verify is incorrect.' },
  NO_DATA: { code: '9994', message: 'No Data or end of list data.' },
  USER_NOT_VALIDATED: { code: '9995', message: 'User is not validated.' },
  USER_EXISTED: { code: '9996', message: 'User existed.' },
  METHOD_INVALID: { code: '9997', message: 'Method is invalid.' },
  TOKEN_INVALID: { code: '9998', message: 'Token is invalid.' },
  EXCEPTION: { code: '9999', message: 'Exception error.' },
} as const;

// Type helper
export type ResponseCode = (typeof RESPONSE)[keyof typeof RESPONSE]['code'];
export type ResponseMessage =
  (typeof RESPONSE)[keyof typeof RESPONSE]['message'];
